...
class UserRegisteredEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.USER_REGISTERED, description="User registration event")
    ...
    ip_address: Optional[str] = Field(None, description="IP address used for registration")
...
class UserLogoutEvent(BaseEvent):
    ...
    # Context
    ip_address: Optional[str] = Field(None, description="IP address at logout")
...
class UserProfileUpdatedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.USER_PROFILE_UPDATED, description="User profile update event")
    
    # Profile changes
    fields_updated: list[str] = Field(..., description="List of profile fields that were updated")
    old_values: Dict[str, Any] = Field(default_factory=dict, description="Previous values of updated fields")
...
class UserDeletedEvent(BaseEvent):
    ...
    # Deletion context
    deletion_reason: Optional[str] = Field(None, description="Reason for account deletion")
    deletion_method: str = Field(..., description="How account was deleted (user request, admin, etc.)")
