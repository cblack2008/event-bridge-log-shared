...
class InventoryLowStockEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.INVENTORY_LOW_STOCK, description="Low stock alert event")
...
class InventoryOutOfStockEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.INVENTORY_OUT_OF_STOCK, description="Out of stock event")
    ...
    estimated_revenue_loss: Optional[Decimal] = Field(None, description="Estimated revenue loss due to stockout")
    ...
    estimated_restock_date: Optional[datetime] = Field(None, description="Estimated date when stock will be available")
    restock_quantity: Optional[int] = Field(None, description="Quantity expected in restock")
...
class InventoryRestockedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.INVENTORY_RESTOCKED, description="Inventory restocked event")
    ...
    # Restock context
    restock_method: str = Field(..., description="How restock was performed (purchase, transfer, return, etc.)")
    supplier_id: Optional[str] = Field(None, description="Supplier identifier if applicable")
    purchase_order_id: Optional[str] = Field(None, description="Purchase order identifier if applicable")
    ...
    lot_number: Optional[str] = Field(None, description="Lot number for tracking")
    expiration_date: Optional[datetime] = Field(None, description="Product expiration date if applicable")
    
    # Cost information
    unit_cost: Optional[Decimal] = Field(None, description="Unit cost of restocked items")
    total_cost: Optional[Decimal] = Field(None, description="Total cost of restock")
