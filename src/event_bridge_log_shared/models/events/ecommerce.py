...
class ProductViewedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.PRODUCT_VIEWED, description="Product view event")
    ...
    # View context
    view_duration: Optional[int] = Field(None, description="Time spent viewing in seconds")
    page_location: Optional[str] = Field(None, description="Page where product was viewed")
...
class ProductSearchedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.PRODUCT_SEARCHED, description="Product search event")
    ...
    # Search context
    search_source: str = Field(..., description="Where search was initiated (header, category, etc.)")
...
class CartItemAddedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.CART_ITEM_ADDED, description="Cart item added event")
...
class CartItemRemovedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.CART_ITEM_REMOVED, description="Cart item removed event")
...
class CartAbandonedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.CART_ABANDONED, description="Cart abandoned event")
...
class OrderCreatedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.ORDER_CREATED, description="Order created event")
    ...
    class Config:
        ...
        schema_extra = {
            "example": {
                ...
                "items": [
                    {"product_id": "prod_123", "name": "Wireless Headphones", "quantity": 2, "price": "99.99"},
                    {"product_id": "prod_456", "name": "Phone Case", "quantity": 1, "price": "19.99"}
                ],
                "item_count": 2,
                "customer_email": "john.doe@example.com",
                "shipping_address": {"street": "123 Main St", "city": "Anytown", "state": "CA", "zip": "12345"},
                "billing_address": {"street": "123 Main St", "city": "Anytown", "state": "CA", "zip": "12345"},
...
class OrderPaidEvent(BaseEvent):
    ...
    processing_time_ms: Optional[int] = Field(None, description="Payment processing time in milliseconds")
...
class OrderShippedEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.ORDER_SHIPPED, description="Order shipped event")
    ...
    # Package information
    package_weight: Optional[Decimal] = Field(None, description="Package weight")
    package_dimensions: Optional[Dict[str, Any]] = Field(None, description="Package dimensions")
...
class OrderDeliveredEvent(BaseEvent):
    ...
    event_type: EventType = Field(default=EventType.ORDER_DELIVERED, description="Order delivered event")
    ...
    delivery_rating: Optional[int] = Field(None, description="Delivery rating (1-5)")
    delivery_feedback: Optional[str] = Field(None, description="Delivery feedback")
